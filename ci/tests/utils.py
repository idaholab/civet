...
def github_config(**kwargs):
    return base_git_config(
        host_type=settings.GITSERVER_GITHUB, icon_class="dummy github class", **kwargs
    )


def gitlab_config(**kwargs):
    return base_git_config(
        host_type=settings.GITSERVER_GITLAB, icon_class="dummy gitlab class", **kwargs
    )
...
def create_event(
    ...
    return models.Event.objects.get_or_create(
        head=c1, base=c2, cause=cause, build_user=user
    )[0]
...
def create_pr(title="testTitle", number=1, url="http", repo=None, server=None):
    ...
    return models.PullRequest.objects.get_or_create(
        repository=repo, number=number, title=title, url=url
    )[0]
...
def create_step(name="testStep", filename="default.sh", recipe=None, position=0):
    ...
    return models.Step.objects.get_or_create(
        recipe=recipe, name=name, position=position, filename=filename
    )[0]
...
def create_recipe_environment(name="testEnv", value="testValue", recipe=None):
    if not recipe:
        recipe = create_recipe()
    return models.RecipeEnvironment.objects.get_or_create(
        name=name, value=value, recipe=recipe
    )[0]
...
def create_step_environment(name="testEnv", value="testValue", step=None):
    if not step:
        step = create_step()
    return models.StepEnvironment.objects.get_or_create(
...
def create_job(recipe=None, event=None, config=None, user=None):
    ...
    return models.Job.objects.get_or_create(config=config, recipe=recipe, event=event)[
        0
    ]
...
def create_prestepsource(filename="default.sh", recipe=None):
    if not recipe:
        recipe = create_recipe()
    return models.PreStepSource.objects.get_or_create(recipe=recipe, filename=filename)[
        0
    ]
...
def create_step_result(
    ...
    result, created = models.StepResult.objects.get_or_create(
        job=job,
        name=step.name,
