...
class DBCompare(object):
    def create_default_recipes(self, server_type=settings.GITSERVER_GITHUB):
        ...
        self.build_user = utils.create_user_with_token(
            name="moosebuild", server=self.server
        )
        ...
        pr1 = utils.create_recipe(
            name="PR With Dep", user=self.build_user, repo=self.repo
        )
        pr1.depends_on.add(pr)
        push = utils.create_recipe(
            name="Push Base",
    ...
    def recipe_deps_count(self):
        count = 0
        for r in (
            models.Recipe.objects.exclude(depends_on=None)
    ...
    def set_counts(self):
        ...
        self.num_push_recipes = models.Recipe.objects.filter(
            cause=models.Recipe.CAUSE_PUSH
        ).count()
        self.num_pr_recipes = models.Recipe.objects.filter(
            cause=models.Recipe.CAUSE_PULL_REQUEST
        ).count()
        self.num_manual_recipes = models.Recipe.objects.filter(
            cause=models.Recipe.CAUSE_MANUAL
        ).count()
        self.num_pr_alt_recipes = models.Recipe.objects.filter(
            cause=models.Recipe.CAUSE_PULL_REQUEST_ALT
        ).count()
        self.num_release_recipes = models.Recipe.objects.filter(
            cause=models.Recipe.CAUSE_RELEASE
        ).count()
        self.num_canceled = models.Job.objects.filter(
            status=models.JobStatus.CANCELED
        ).count()
        self.num_events_canceled = models.Event.objects.filter(
            status=models.JobStatus.CANCELED
        ).count()
    ...
    def compare_counts(
        ...
        num_push_recipes=0,
        num_pr_recipes=0,
        num_manual_recipes=0,
        num_pr_alt_recipes=0,
        canceled=0,
        invalidated=0,
        active=0,
        num_steps=0,
        ...
        self.assertEqual(
            self.num_jobs_ready + ready, models.Job.objects.filter(ready=True).count()
        )
        self.assertEqual(
            self.num_jobs_active + active,
            models.Job.objects.filter(active=True).count(),
        )
        ...
        self.assertEqual(
            self.num_current + current,
            models.Recipe.objects.filter(current=True).count(),
        )
        ...
        self.assertEqual(
            self.num_active_repos + active_repos,
        ...
        self.assertEqual(
            self.num_active_branches + active_branches,
        ...
        self.assertEqual(
            self.num_push_recipes + num_push_recipes,
            models.Recipe.objects.filter(cause=models.Recipe.CAUSE_PUSH).count(),
        )
        self.assertEqual(
            self.num_pr_recipes + num_pr_recipes,
            models.Recipe.objects.filter(
                cause=models.Recipe.CAUSE_PULL_REQUEST
            ).count(),
        )
        self.assertEqual(
            self.num_manual_recipes + num_manual_recipes,
            models.Recipe.objects.filter(cause=models.Recipe.CAUSE_MANUAL).count(),
        )
        self.assertEqual(
            self.num_pr_alt_recipes + num_pr_alt_recipes,
            models.Recipe.objects.filter(
                cause=models.Recipe.CAUSE_PULL_REQUEST_ALT
            ).count(),
        )
        self.assertEqual(
            self.num_release_recipes + num_release_recipes,
        ...
        self.assertEqual(
            self.num_events_canceled + events_canceled,
            models.Event.objects.filter(status=models.JobStatus.CANCELED).count(),
        )
        self.assertEqual(
            self.num_invalidated + invalidated,
            models.Job.objects.filter(invalidated=True).count(),
        )
        self.assertEqual(self.num_steps + num_steps, models.Step.objects.count())
        self.assertEqual(
            self.num_step_envs + num_step_envs, models.StepEnvironment.objects.count()
        )
        self.assertEqual(
            self.num_recipe_envs + num_recipe_envs,
            models.RecipeEnvironment.objects.count(),
        )
        self.assertEqual(
            self.num_prestep + num_prestep, models.PreStepSource.objects.count()
        )
        self.assertEqual(
            self.num_pr_alt_count + num_pr_alts, self.pr_alternates_count()
        )
        self.assertEqual(
            self.num_repo_prefs_count + repo_prefs, self.repo_prefs_count()
        )
        self.assertEqual(self.num_clients + num_clients, models.Client.objects.count())
        self.assertEqual(
            self.num_changelog + num_changelog, models.JobChangeLog.objects.count()
        )
        self.assertEqual(
            self.num_events_completed + num_events_completed,
            models.Event.objects.filter(complete=True).count(),
        )
        self.assertEqual(
            self.num_jobs_completed + num_jobs_completed,
            models.Job.objects.filter(complete=True).count(),
        )
        self.assertEqual(
            self.num_viewable_teams + viewable_teams,
            models.RecipeViewableByTeam.objects.count(),
        )
        self.assertEqual(
            self.num_badges + badges, models.RepositoryBadge.objects.count()
        )
