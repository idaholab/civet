...
class Tests(DBTester.DBTester):
    ...
    def test_no_recipes(self):
        ...
        c1_data = GitCommitData.GitCommitData(
            "no_exist", "no_exist", "no_exist", "1", "", self.build_user.server
        )
        push.base_commit = c1_data
        self.set_counts()
        push.save()
    ...
    def test_save(self):
        ...
        base = models.Recipe.objects.filter(
            cause=models.Recipe.CAUSE_PUSH, depends_on=None
        )
        self.assertEqual(base.count(), 1)
        base = base.first()
        with_dep = models.Recipe.objects.filter(cause=models.Recipe.CAUSE_PUSH).exclude(
            depends_on=None
        )
    ...
    def test_auto_cancel_on_push(self):
        ...
        self.assertEqual(
            [models.JobStatus.NOT_STARTED, models.JobStatus.CANCELED], js_status
        )
        old_ev.refresh_from_db()
        ...
        INSTALLED_GITSERVERS=[
            utils.github_config(
                repo_settings={
                    "owner/repo": {
                        "branch_settings": {
                            "devel": {"auto_cancel_push_events_except_current": True}
                        }
                    }
                }
            )
    ...
    def test_auto_cancel_except_current(self):
        ...
        js_status = sorted([j.status for j in e0.jobs.all()])
        self.assertEqual(
            [models.JobStatus.NOT_STARTED, models.JobStatus.RUNNING], js_status
        )
