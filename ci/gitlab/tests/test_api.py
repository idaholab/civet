...
class Tests(DBTester.DBTester):
    ...
    def test_update_status(self, mock_post):
        ...
        with self.settings(
            ...
            mock_post.return_value = utils.Response(
                status_code=200, content="some content"
            )
            api = self.server.api()
            api.update_status(
                ev.base,
    ...
    def test_get_pr_comments(self, mock_get):
        ...
        c0 = {
            "author": {"username": self.build_user.name},
            "body": "some message",
            "id": 1,
        }
        c1 = {
    ...
    def test_create_or_update_issue(
        self, mock_get, mock_post, mock_put, mock_rget, mock_rput
    ):
        with self.settings(
            INSTALLED_GITSERVERS=[utils.gitlab_config(remote_update=True)]
        ):
