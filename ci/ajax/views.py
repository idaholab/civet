...
def pr_update(request, pr_id):
    ...
    pr_data["events"] = EventsStatus.multiline_events_info(
        pr.events.all(), events_url=True
    )
...
def repo_update(request):
    ...
    """
    if (
        "last_request" not in request.GET
        or "limit" not in request.GET
        or "repo_id" not in request.GET
    ):
        return HttpResponseBadRequest("Missing parameters")
    ...
    for pr in (
        models.PullRequest.objects.filter(
            repository=repo, closed=True, last_modified__gte=dt
        )
        .values("id")
        .all()
    ):
...
def job_results(request):
    ...
    if job.last_modified < dt:
        ...
        # "natural" time
        return JsonResponse(
            {"job_info": job_info, "results": [], "last_request": this_request}
        )
    ...
    return JsonResponse(
        {"job_info": job_info, "results": result_info, "last_request": this_request}
    )
...
def user_open_prs(request, username):
    ...
    repos = RepositoryStatus.get_user_repos_with_open_prs_status(
        username, dt, filter_repo_ids=viewable_repos
    )
