...
class Tests(ClientTester.ClientTester):
    ...
    def test_create_event_summary(self, mock_get, mock_post):
        ...
        with self.settings(
            INSTALLED_GITSERVERS=[
                utils.github_config(post_event_summary=False, remote_update=True)
            ]
        ):
        ...
        with self.settings(
            INSTALLED_GITSERVERS=[
                utils.github_config(post_event_summary=True, remote_update=True)
            ]
        ):
            ...
            utils.update_job(
                j1, status=models.JobStatus.FAILED, complete=True, invalidated=True
            )
    ...
    def test_start_canceled_on_fail(self):
        ...
        repo_name = "%s/%s" % (
            e0.base.branch.repository.user.name,
        ...
        with self.settings(
            INSTALLED_GITSERVERS=[utils.github_config(repo_settings=repo_settings)]
        ):
            # If the job isn't a fail then it shouldn't do anything
            self.set_counts()
    ...
    def test_check_automerge(self, mock_get):
        ...
        git_config = utils.github_config(
            remote_update=True, repo_settings=repo_settings
        )
