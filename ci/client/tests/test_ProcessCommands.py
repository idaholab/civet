...
class Tests(ClientTester.ClientTester):
    ...
    def test_check_post_comment(
        self, mock_get_comments, mock_comment, mock_remove, mock_edit
    ):
        result = self.create_step_result(
            "PREVIOUS_LINE\nCIVET_CLIENT_POST_MESSAGE=My message\nMore text\n"
        )
        self.assertIs(
            ProcessCommands.check_post_comment(
                result.job, result.position, False, False
            ),
            True,
        )
        ...
        self.assertIs(
            ProcessCommands.check_post_comment(
                result.job, result.position, False, True
            ),
            True,
        )
    ...
    def test_process_commands(self, mock_post, mock_submodule):
        ...
        utils.create_step_environment(
            name="CIVET_SERVER_POST_REMOVE_OLD", value="1", step=step
        )
        ProcessCommands.process_commands(job)
        self.assertEqual(mock_post.call_count, 2)
        self.assertEqual(mock_submodule.call_count, 1)
        ...
        utils.create_step_environment(
            name="CIVET_SERVER_POST_EDIT_EXISTING", value="1", step=step
        )
        ProcessCommands.process_commands(job)
