...
class RecipeCreateView(RecipeBaseView, CreateView):
  ...
  def get(self, request, *args, **kwargs):
    ...
    if recipe:
      ...
      step_initial = [{'name': step.name, 'position': step.position, 'filename': step.filename, 'allowed_to_fail': step.allowed_to_fail, 'abort_on_failure': step.abort_on_failure } for step in recipe.steps.all() ]
