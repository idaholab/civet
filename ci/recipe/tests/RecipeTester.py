...
class RecipeTester(DBTester.DBTester):
    ...
    def create_recipe_in_repo(
        self, recipes_dir, test_recipe, repo_recipe, hostname=None
    ):
    ...
    def write_script_to_repo(self, recipes_dir, file_data, script_name):
        ...
        subprocess.check_output(
            ["git", "commit", "-m", "Added %s" % fname], cwd=recipes_dir
        )
        return fname
    ...
    def remove_recipe_from_repo(self, recipes_dir, script_name):
        ...
        subprocess.check_output(
            ["git", "commit", "-m", "Remove %s" % fname], cwd=recipes_dir
        )
    ...
    def write_to_repo(self, recipes_dir, file_data, repo_recipe):
        ...
        subprocess.check_output(["git", "add", fname], cwd=recipes_dir)
        subprocess.check_output(
            ["git", "commit", "-m", "Added %s" % repo_recipe], cwd=recipes_dir
        )
        return fname
    ...
    def create_records(self, recipe, branch):
        ...
        info["build_user"] = test_utils.create_user_with_token(
            name=recipe["build_user"]
        )
        info["repository"] = test_utils.create_repo(
            user=info["owner"], name=recipe["repository_name"]
        )
        info["branch"] = test_utils.create_branch(repo=info["repository"], name=branch)
        return info

    def create_default_recipes(
        self, recipes_dir, server_type=settings.GITSERVER_GITHUB
    ):
        hostname = "github.com"
        if server_type == settings.GITSERVER_GITLAB:
            hostname = "gitlab.com"

        self.recipe_file = self.create_recipe_in_repo(
            "recipe_all.cfg", "recipe.cfg", hostname=hostname
        )
        self.recipe_pr_file = self.create_recipe_in_repo(
            "pr_dep.cfg", "pr_dep.cfg", hostname=hostname
        )
        self.recipe_push_file = self.create_recipe_in_repo(
            "push_dep.cfg", "push_dep.cfg", hostname=hostname
        )
        self.server = test_utils.create_git_server(host_type=server_type)
        self.build_user = test_utils.create_user_with_token(
