...
class scheduleConfig(AppConfig):
    ...
    def schedulePinger():
        ...
        while True:
            ...
            dbRecipes = models.Recipe.objects.filter(
            ...
            for r in dbRecipes:
                ...
                if (
                    r.last_scheduled == datetime.fromtimestamp(0, tz=pytz.UTC)
                    and not r.schedule_initial_run
                ):
                    r.last_scheduled = now - timedelta(seconds=1)
                    r.save()

                c = croniter(
                    r.scheduler, start_time=r.last_scheduled.astimezone(local_tz)
                )
                ...
                if next_job_run_time <= now:
                    ...
                    if latest:  # likely need to add exception checks for this!
                        ...
                        r.save()
                        logger.info(
                            "SCHEDULER: Running scheduled job {} on {}, soonest run time is {}, next run time is {}".format(
                                r.name,
                        ...
                        )  # magically add the job through a blackbox
                else:
                    logger.debug(
                        "SCHEDULER: Not running recipe {} on {}, next run time is {}".format(
                            r.name,
                            branch.repository.name,
                            format_time(next_job_run_time),
                        )
            ...
            logger.debug(
                "SCHEDULER: Sleeping for {} sec until {}".format(dt, next_run_time)
            )
            time.sleep(dt)
        ...
        if os.environ.get("RUN_MAIN", None) != "true":
            try:
                ...
                if uwsgi.worker_id() == 1:
                    threading.Thread(
                        target=scheduleConfig.schedulePinger, args=(), daemon=True
                    ).start()
            except ImportError:
                pass
