...
def call_daemon(client, cmd):
    ...
    pfile = os.path.join(
        home, "civet_client_%s.pid" % client.client_info["client_name"]
    )
    client_daemon = ClientDaemon(
        pfile,
        stdout=client.client_info["log_file"],
        stderr=client.client_info["log_file"],
    )
...
def commandline_client(args):
    ...
    parser.add_argument(
        "--log-file", dest="log_file", help="Filename to append the log to"
    )
    parser.add_argument(
        "--insecure",
        dest="insecure",
    ...
    if (
        ...
        if (
            not parsed.env or "BUILD_ROOT" not in parsed.env
        ) and "BUILD_ROOT" in os.environ:
