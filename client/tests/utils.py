...
def create_step_dict(script_sleep=2, num=1):
    step = {
        "environment": {"foo": "bar", "step_var_with_root": "BUILD_ROOT/foo"},
...
def create_job_dict(num_steps=1, pk=1):
    job = {
        "environment": {
            "CIVET_BASE_REPO": "base repo",
            "var_with_root": "BUILD_ROOT/bar",
        },
...
def check_step(test_obj, step, client_info, mock_obj):
    ...
    line = "%s\\n%s\\n%s\\n" % (env_line, start, done)
    test_obj.assertTrue(
        in_call_args(mock_obj, server_url("start_step", client_info, step), 0)
    )
    test_obj.assertTrue(
        in_call_args(mock_obj, server_url("update_step", client_info, step), 0)
    )
    test_obj.assertTrue(
...
def create_client_job(recipe_dir, name="TestJob", sleep=1, n_steps=3, extra_script=""):
    ...
    utils.create_recipe_environment(
        name="GLOBAL_NAME", value="BUILD_ROOT/global", recipe=recipe
    )
    count = 0
    for s in [f"step{i}".format(i) for i in range(n_steps)]:
        ...
        utils.create_step_environment(
            name="STEP_NAME", value="BUILD_ROOT/%s" % s, step=step
        )
...
def check_complete_step(self, job, client, result, extra_step_msg=""):
    ...
    output = "{0} {1} {2}\nstart {1}:{3}\nend {1}:{3}\n{4}".format(
        global_var, job.recipe.name, step_var, result.name, extra_step_msg
    )
