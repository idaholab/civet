...
class Tests(LiveClientTester.LiveClientTester):
    ...
    def create_job(
        self, client, recipes_dir, name, sleep=1, n_steps=3, extra_script=""
    ):
        job = utils.create_client_job(
            recipes_dir,
            name=name,
            sleep=sleep,
            n_steps=n_steps,
    ...
    def setup_post_completed_commands(self, client, tmpdir):
        client.client_info["post_step_command"] = (
            f'printf "CIVET_STEP_COMPLETED=$CIVET_STEP_COMPLETED" > {tmpdir}/post_step'
        )
        client.client_info["post_job_command"] = (
            f'printf "CIVET_JOB_COMPLETED=$CIVET_JOB_COMPLETED" > {tmpdir}/post_job'
        )

    def check_post_completed_commands(self, tmpdir, fail):
        value = "0" if fail else "1"
        self.assertEqual(
            open(f"{tmpdir}/post_step").read(), f"CIVET_STEP_COMPLETED={value}"
        )
        self.assertEqual(
            open(f"{tmpdir}/post_job").read(), f"CIVET_JOB_COMPLETED={value}"
        )
    ...
    def test_run_job_invalidated_basic(self):
        with test_utils.RecipeDir() as recipe_dir:
            with tempfile.TemporaryDirectory() as tmp:
                c, job = self.create_client_and_job(
                    recipe_dir, "JobInvalidated", sleep=40
                )
                self.setup_post_completed_commands(c, tmp)
    ...
    def test_manage_build_root(self):
        with test_utils.RecipeDir() as recipe_dir:
            ...
            for job in jobs:
                utils.check_complete_job(
                    self, job, c, n_steps=1, extra_step_msg="BUILD_ROOT_EXISTS_EMPTY\n"
                )
    ...
    def test_deprecated_environment(self):
        with test_utils.RecipeDir() as recipe_dir:
            ...
            utils.check_complete_job(
                self, job, c, n_steps=1, extra_step_msg="FOO=bar\n"
            )
    ...
    def run_stage_command(self, stage, fail=False):
        with test_utils.RecipeDir() as recipe_dir:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                c, _ = self.create_client_and_job(
                    recipe_dir, "RunSuccess", sleep=0, n_steps=n_steps
                )
                self.set_counts()
                ...
                if fail:
                    ...
                    if run_steps:
                        c.client_info[f"{stage}_command"] = (
                            f'printf "{stage}_$CIVET_STEP_NUM" > {tmp}/{stage}_$CIVET_STEP_NUM'
                        )
                    else:
                        c.client_info[f"{stage}_command"] = (
                            f'printf "{stage}" > {tmp}/{stage}'
                        )
                ...
                if fail:
                    ...
                    if run_steps:
                        for step in range(n_steps):
                            self.assertEqual(
                                f"{stage}_{step}",
                                open(f"{tmp}/{stage}_{step}", "r").read(),
                            )
                    else:
