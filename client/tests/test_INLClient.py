        ...
        for stage in [
            ...
            with tempfile.NamedTemporaryFile() as tmp:
                c = self.create_client(
                    self.default_args + [stage_arg, f'printf "foo=bar" > {tmp.name}']
                )["client"]
            ...
            with tempfile.NamedTemporaryFile() as tmp:
                c = self.create_client(
                    self.default_args + [stage_arg, f"env > {tmp.name}"]
                )["client"]
                self.assertTrue(c.run_stage_command(stage, env={"FOO": "BAR"}))
                self.assertIn("FOO=BAR", open(tmp.name, "r").read())
            c.check_stage_commands()
            # Command failed
            c = self.create_client(self.default_args + [stage_arg, "exit 123"])[
                "client"
            ]
